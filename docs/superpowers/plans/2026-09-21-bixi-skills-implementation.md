...
**Status:** 2026-09-21 首版本地开发与验收完成，进度 100%。Task 1–9 的文件、静态校验、10 次独立安装、双客户端读取与调用、15 个加载后场景及基线对照审计均已完成。真实 F2 通过双模运行权限与审计验收，D2 取得回归测试失败到通过的证据。用户在验收后授权统一提交并推送至 GitHub，各阶段提交检查点不再单独执行。规格见[设计文档](../specs/2026-09-21-bixi-skills-design.md)，历史失败及验证限制见[评测记录](../../../skills/evals/README.md)。
...
- 下文提交命令是获准提交时的检查点，不表示技能自动获得提交授权；未要求提交则保留文件和验证结果，不反复询问。本次依据验收后的明确授权，统一提交技能、校验器及文档；各阶段示例提交命令不单独执行。
...
`architecture-boundary.md` 写入 A1 的 api→biz 与重复 Controller、A2 的 single Feign 回环、A3 的合法适配器不误报；逐项断言问题和文件证据，与 Task 8 的固定场景一致。
...
测试脚本用临时目录生成六类基础 fixture，并按实际缺陷增加回归 fixture，并直接调用校验器导出的 `validate(root)`：
...
预期：fixture 测试全部通过；Task 2–6 已完成后，当前完整包 CLI 通过。缺少 Skill/评测的错误由坏包 fixture 验证，不能跳过检查。

- [x] **Step 4: 校验触发描述和引用边界**

校验器只检查结构与登记引用；触发是否冲突、技能是否做出正确判断交由 Task 8 的行为评测。README 不得声称脚本能证明客户端自动触发、代码实现质量或双模运行成功。

- **Step 5: 提交任务 7（纳入 Task 9 最终提交，不单独执行）**
...
- **Step 7: 提交任务 8 的脱敏评测资料（纳入 Task 9 最终提交，不单独执行）**
...
- [x] **Step 2: 为每个技能执行 Claude Code 项目级安装**
...
如果改动只包含技能文档，不机械执行 Bixi 后端/前端 CI；如果评测副本改动涉及 Bixi 行为，CI 和运行验收在副本中单独报告。`codegraph sync .` 在 `.codegraph/` 存在时执行；无目录则不创建索引。存在目录但不可用时记录实际失败，不伪造通过。

- [x] **Step 5: 更新实际状态并完成检查点**

将 README、设计文档和计划中的“待完成”改为实际状态；只填写已经运行的命令和退出码。若双客户端、F2、D2 或运行级门禁阻塞，保留阻塞项，不能打上完整验收标记。

- [x] **Step 6: 最终提交（用户已明确授权，并追加 GitHub 推送）**
