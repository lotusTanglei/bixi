	...
	<div class="layout-padding">
		<div class="layout-padding-auto layout-padding-view">
			...
			<el-table :data="state.dataList" v-loading="state.loading || busy" border :empty-text="done ? '暂无已办任务' : '暂无待办任务'">
				...
				<el-table-column v-else label="状态" width="110"><template #default="{ row }">{{ row.delegationState === 'PENDING' ? '待交回委派' : row.assignee ? '待审批' : '待认领' }}</template></el-table-column>
				<el-table-column label="操作" :width="done ? 150 : 340" fixed="right">
					<template #default="{ row }">
						...
						<template v-else>
							<el-button v-if="!row.assignee" v-auth="'workflow_task_edit'" text type="primary" @click="claimTask(row)">认领</el-button>
							<el-button v-else-if="row.delegationState === 'PENDING'" v-auth="'workflow_task_edit'" text type="primary" @click="resolveTask(row)">处理并交回</el-button>
							<template v-else>
...
import { todoPageList, donePageList } from '/@/api/workflow/task';
...
const run = async (operation: string, row: any, payload: () => Promise<Record<string, unknown>>, message: string) => {
...
const claimTask = (row: any) => run('CLAIM', row, async () => ({}), '任务已认领');
const resolveTask = (row: any) => run('RESOLVE', row, async () => {
